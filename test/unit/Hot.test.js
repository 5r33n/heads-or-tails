...
!developmentChains.includes(network.name)
    ...
    : describe("HoT Unit Tests", function () {
          let hot, vrfCoordinatorV2Mock, hotEntranceFee, deployer, interval
          ...
          describe("enterHot", function () {
              it("reverts when you don't pay enuff", async function () {
                  await expect(hot.enterHot()).to.be.revertedWith("Hot__NotEnoughETHEntered")
              })
              it("records players when they enter", async function () {
                  await hot.enterHot({ value: hotEntranceFee })
                  const playerFromContract = await hot.getPlayer(0)
                  assert.equal(playerFromContract, deployer)
              })
              it("emits event on enter", async function () {
                  await expect(hot.enterHot({ value: hotEntranceFee })).to.emit(hot, "HotEnter")
              })
              it("doesn't allow entrance when hot is in calculating state", async function () {
                  ...
                  await expect(hot.enterHot({ value: hotEntranceFee })).to.be.revertedWith(
                      "Hot__NotOpen"
                  )
              })
          })
          ...
          describe("fulfillRandomWords", function () {
              ...
              it("picks a winner, resets the lottery, and sends the money", async function () {
                  ...
                  for (
                      ...
                      await accountConnectedHot.enterHot({ value: hotEntranceFee })
                  }
